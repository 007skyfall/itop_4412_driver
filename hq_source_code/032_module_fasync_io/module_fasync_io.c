...
ssize_t demo_write(struct file * filep, const char __user * buffer, size_t size, loff_t * offlen)
{
	...
	kill_fasync(&fasyncqueue, SIGIO, POLLIN|POLLRDNORM);

	return  size;
}

unsigned int demo_poll(struct file * filep, struct poll_table_struct * wait)
{
	unsigned int mask = 0;
...
{
	printk("%s,%d\n", __func__, __LINE__);
	return  fasync_helper(fd, filep, on, &fasyncqueue);
}
...
static int __init demo_init(void)
{
	...
	ret = register_chrdev_region(devno, count, "module_fasync_io");
	if(ret)
	{
